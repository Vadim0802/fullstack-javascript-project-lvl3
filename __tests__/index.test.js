...
beforeEach(async () => {
  ...
  noAccessDownloadDirectory = await fs.mkdtemp(
    path.join(os.tmpdir(), "page-loader-no-access")
  );
...
test("page-loader with existing page", async () => {
  assets.forEach((asset) => {
    scope.get(asset).reply(200, fs.readFile(getFixturePath(asset), "utf-8"));
  });
...
test("page-loader should fail and show EACCES error", async () => {
  await expect(pageLoader(link, noAccessDownloadDirectory)).rejects.toThrow(
    "EACCES"
  );
});
