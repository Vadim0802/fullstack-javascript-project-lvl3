...
export default (link, dirpath) => {
  ...
  return axios
    .get(link)
    .then(({ data }) =>
      convertAssetUrls(data, urlInstance.origin, assetsDirname)
    )
    .then(({ assetUrls, html }) => {
      const prettifyHtml = prettier.format(html, { parser: "html" });
      return fs
        .writeFile(resourceFilepath, prettifyHtml, "utf-8")
        .then(() => assetUrls);
    })
    .then((assetUrls) =>
      fs
        .mkdir(path.join(dirpath, assetsDirname))
